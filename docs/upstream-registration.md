...
SourceForge documents that SHA1 and MD5 are shown by clicking the information icon beside a file:
...
SHA-1 values contain 40 hexadecimal characters. MD5 values contain 32. Do not calculate these four values yourself: they must be copied from the upstream record. The scripts calculate SHA-256 and file sizes after downloading.
...
The tool downloads both archives, verifies the supplied SHA-1 and MD5 values, calculates SHA-256 and sizes, and writes `config/upstream/<version>.json`. It refuses to overwrite an existing config.
...
Before committing the config, check its version, filenames, URLs, sizes, digests, and `SOURCE_DATE_EPOCH`. Then run the full workflow for the new upstream version.
